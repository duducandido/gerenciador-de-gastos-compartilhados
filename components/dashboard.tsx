  ...
  return (
    <main className="min-h-screen bg-[#f7f8fa] text-[#20252b]">
      ...
      <div className="mx-auto flex max-w-[1320px]">
        ...
        {activeTab === 'calendar' && <section className="min-w-0 flex-1 px-5 pb-12 pt-8 lg:px-10 lg:pt-12"><div className="mb-9"><p className="mb-2 text-sm font-medium text-[#8a938f]">Planejamento mensal</p><h1 className="text-3xl font-bold tracking-[-0.04em] text-[#203f36] sm:text-[38px]">Calendário</h1><p className="mt-2 text-sm text-[#7c8881]">Visualize vencimentos, contas recorrentes e lembretes.</p></div><div className="grid gap-6 lg:grid-cols-[1.25fr_.75fr]"><div className="rounded-2xl border border-[#e5e9e7] bg-white p-6"><div className="mb-6 flex items-center justify-between"><div><p className="text-sm font-semibold text-[#203f36]">Junho 2025</p><p className="mt-1 text-xs text-[#8a938f]">Contas planejadas</p></div><button onClick={() => setShowExpense(true)} className="flex items-center gap-2 rounded-xl bg-[#c8f169] px-3 py-2 text-xs font-bold text-[#203f36]"><Plus size={15} /> Lançar conta</button></div><div className="grid grid-cols-7 gap-2 text-center text-xs"><div className="col-span-7 grid grid-cols-7 pb-2 font-bold text-[#9aa29e]">{['D','S','T','Q','Q','S','S'].map((day, index) => <span key={`${day}-${index}`}>{day}</span>)}</div>{Array.from({ length: 30 }, (_, index) => { const day = index + 1; const highlighted = [2, 5, 10, 15, 20, 25].includes(day); return <button key={day} className={`relative rounded-xl p-3 font-semibold transition hover:bg-[#f1f8e6] ${day === 3 ? 'bg-[#203f36] text-white' : 'text-[#53635a]'}`}>{day}{highlighted && <span className={`absolute bottom-1 left-1/2 h-1 w-1 -translate-x-1/2 rounded-full ${day === 3 ? 'bg-[#c8f169]' : 'bg-[#9bc65b]'}`} />}</button> })}</div></div><div className="rounded-2xl border border-[#e5e9e7] bg-white p-6"><h2 className="text-lg font-bold text-[#203f36]">Próximos lembretes</h2><div className="mt-5 space-y-3"><Reminder title="Aluguel" date="Dia 05" amount="R$ 1.850,00"/><Reminder title="Internet" date="Dia 10" amount="R$ 119,90"/><Reminder title="Cartão de crédito" date="Dia 15" amount="R$ 640,00"/></div><button className="mt-5 w-full rounded-xl border border-[#dce4df] py-3 text-sm font-bold text-[#52645a] hover:bg-[#f8faf7]">Gerenciar lembretes</button></div></div></section>}
