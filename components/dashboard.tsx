...
import { createPayableBill, createSavingsGoal, payNextInstallment, contributeToSavingsGoal, updateHouseholdSettings, updateProfileAvatar } from '@/app/actions/household'
import {
  ArrowDownLeft,
  ArrowUpRight,
  Bell,
...
export default function Page({ initialProfileName, initialHouseholdName, initialAvatarImage, initialAccountCreatedAt, initialSavings, initialPayableBills }: DashboardProps) {
  ...
  {activeTab === 'reports' && <section className="min-w-0 flex-1 px-5 pb-12 pt-8 lg:px-10 lg:pt-12"><div className="mb-9 flex items-end justify-between gap-4"><div><p className="mb-2 text-sm font-medium text-[#8a938f]">Análise da conta</p><h1 className="text-3xl font-bold tracking-[-0.04em] text-[#203f36] sm:text-[38px]">Relatórios</h1><p className="mt-2 text-sm text-[#7c8881]">Entendam para onde o dinheiro está indo.</p></div><button onClick={exportExpenses} className="rounded-xl border border-[#dce4df] bg-white px-4 py-3 text-sm font-bold text-[#52645a]">Exportar Excel</button></div><div className="grid gap-4 md:grid-cols-3">{['Casa', 'Alimentação', 'Transporte'].map((category, index) => { const categoryTotal = expenseItems.filter((item) => item.category === category).reduce((sum, item) => sum + item.amount, 0); const percent = total > 0 ? Math.round((categoryTotal / total) * 100) : 0; return <Stat key={category} label={category} value={formatCurrency(categoryTotal)} percent={`${percent}%`} color={['bg-[#7ea8a0]', 'bg-[#e7a65b]', 'bg-[#9bc65b]'][index]} /> })}</div><div className="mt-6 rounded-2xl border border-[#e5e9e7] bg-white p-6"><h2 className="text-lg font-bold text-[#203f36]">Divisão por categoria</h2><div className="mt-6 space-y-5">{['Casa', 'Alimentação', 'Transporte', 'Lazer'].map((category, index) => { const categoryTotal = expenseItems.filter((item) => item.category === category).reduce((sum, item) => sum + item.amount, 0); const percent = total > 0 ? Math.round((categoryTotal / total) * 100) : 0; return <ReportBar key={category} label={category} value={formatCurrency(categoryTotal)} percent={`${percent}%`} color={['bg-[#7ea8a0]', 'bg-[#e7a65b]', 'bg-[#9bc65b]', 'bg-[#b68ac5]'][index]} /> })}</div></div></section>}
      ...
      {activeTab === 'members' && <section className="min-w-0 flex-1 px-5 pb-12 pt-8 lg:px-10 lg:pt-12"><div className="mb-9 flex items-end justify-between gap-4"><div><p className="mb-2 text-sm font-medium text-[#8a938f]">Conta compartilhada</p><h1 className="text-3xl font-bold tracking-[-0.04em] text-[#203f36] sm:text-[38px]">Membros</h1><p className="mt-2 text-sm text-[#7c8881]">Controle quem participa da organização financeira.</p></div><button onClick={() => setShowInvite(true)} className="flex items-center gap-2 rounded-xl bg-[#c8f169] px-4 py-3 text-sm font-bold text-[#203f36]"><Share2 size={17} /> Convidar pessoa</button></div><div className="rounded-2xl border border-[#e5e9e7] bg-white p-6"><div className="space-y-3">{members.map((member) => <div key={member.id} className="flex items-center justify-between rounded-xl bg-[#f8faf7] px-4 py-4"><div className="flex items-center gap-3"><span className="flex h-10 w-10 items-center justify-center rounded-full bg-[#f2c9a8] text-xs font-bold text-[#713d22]">{member.name.split(' ').map((part) => part[0]).join('').slice(0, 2).toUpperCase()}</span><div><p className="font-bold text-[#35433d]">{member.name}</p><p className="text-xs text-[#8a938f]">{member.role}</p></div></div>{member.role !== 'Administrador' && <button onClick={() => removeMember(member.id)} className="text-xs font-bold text-[#a33c30] hover:underline">Remover acesso</button>}</div>)}</div><p className="mt-5 rounded-xl bg-[#f1f8e6] px-4 py-3 text-xs leading-relaxed text-[#557932]">Administradores podem convidar pessoas e remover acessos. O histórico financeiro permanece visível para todos os membros.</p></div></section>}
