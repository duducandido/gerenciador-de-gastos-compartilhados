  ...
  return (
    <main className="min-h-screen bg-[var(--app-bg)] text-[var(--app-text)]">
      ...
      <div className="mx-auto flex max-w-[1320px]">
        <aside aria-label="Navegação principal" className={`${mobileMenu ? 'flex' : 'hidden'} fixed inset-0 z-10 w-72 flex-col border-r border-[#e6e9ed] bg-[#f7f8fa] p-5 pt-24 lg:static lg:flex lg:min-h-[calc(100vh-80px)] lg:w-60 lg:bg-transparent lg:p-8 lg:pt-10`}>
        ...
        {activeTab === 'overview' && <section className="min-w-0 flex-1 px-5 pb-12 pt-8 lg:px-10 lg:pt-12">
          ...
          <div className="grid gap-4 md:grid-cols-3">
            <div className="rounded-2xl bg-[#203f36] p-6 text-white shadow-[0_12px_30px_rgba(32,63,54,.13)] md:col-span-2"><div className="flex items-start justify-between"><div><p className="text-sm text-[#a8c0b2]">Total gasto em junho</p><p className="mt-2 text-4xl font-bold tracking-[-0.05em]">{formatCurrency(total)}</p></div><div className="rounded-xl bg-white/10 p-3 text-[#c8f169]"><Wallet size={22} /></div></div><div className="mt-7 flex items-end justify-between"><div><div className="mb-2 flex items-center gap-2 text-xs text-[#a8c0b2]"><span className="flex items-center gap-1 text-[#c8f169]"><ArrowDownLeft size={13} /> 0%</span> vs. mês passado</div><div className="h-2 w-48 overflow-hidden rounded-full bg-white/15"><div className="h-full w-0 rounded-full bg-[#c8f169]" /></div></div><p className="text-right text-xs text-[#a8c0b2]">limite mensal<br /><strong className="text-sm text-white">{formatCurrency(Number(monthlyLimit.replace(',', '.')) || 0)}</strong></p></div></div>
            <div className="rounded-2xl border border-[#e5e9e7] bg-white p-6"><div className="flex items-start justify-between"><div><p className="text-sm text-[#8a938f]">Saldo do mês</p><p className="mt-2 text-2xl font-bold tracking-[-0.04em] text-[#203f36]">{formatCurrency(availableBalance)}</p></div><div className="rounded-xl bg-[#eaf5dd] p-3 text-[#719b37]"><ArrowDownLeft size={21} /></div></div><p className="mt-7 text-xs text-[#8a938f]">disponível depois de gastos, reservas e contas</p><div className="mt-4 grid grid-cols-3 gap-2 text-center text-[11px]"><div className="rounded-lg bg-[#f8faf7] p-2"><span className="block text-[#8a938f]">Gastos</span><strong className="text-[#35433d]">{formatCurrency(total)}</strong></div><div className="rounded-lg bg-[#f8faf7] p-2"><span className="block text-[#8a938f]">Reservado</span><strong className="text-[#35433d]">{formatCurrency(reservedTotal)}</strong></div><div className="rounded-lg bg-[#f8faf7] p-2"><span className="block text-[#8a938f]">A pagar</span><strong className="text-[#35433d]">{formatCurrency(billsTotal)}</strong></div></div><div className="mt-2 flex justify-between text-xs font-semibold text-[#4d6256]"><span>{monthlyLimit ? `${Math.round(((total + reservedTotal + billsTotal) / Math.max(1, Number(monthlyLimit.replace(',', '.')))) * 100)}% usado` : 'Defina seu limite mensal'}</span><span>{monthlyLimit ? formatCurrency(Number(monthlyLimit.replace(',', '.'))) : 'Sem limite'}</span></div><div className="mt-2 h-2 overflow-hidden rounded-full bg-[#edf0ee]"><div className="h-full rounded-full bg-[#9bc65b]" style={{ width: `${monthlyLimit ? Math.min(100, Math.round(((total + reservedTotal + billsTotal) / Math.max(1, Number(monthlyLimit.replace(',', '.')))) * 100)) : 0}%` }} /></div></div>
          </div>
