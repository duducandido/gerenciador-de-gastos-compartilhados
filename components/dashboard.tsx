    ...
    <main className="min-h-screen bg-[var(--app-bg)] text-[var(--app-text)]">
      ...
      <div className="mx-auto flex max-w-[1320px]">
        ...
        {activeTab === 'expenses' && <section className="min-w-0 flex-1 px-5 pb-12 pt-8 lg:px-10 lg:pt-12">
          ...
          <div className="mt-8 rounded-2xl border border-[#e5e9e7] bg-white p-5 sm:p-6"><div className="flex flex-col justify-between gap-3 sm:flex-row sm:items-center"><h2 className="text-lg font-bold text-[#203f36]">Todos os gastos</h2><label className="flex items-center gap-2 text-xs font-semibold text-[#7c8881]">Filtrar por<select value={expenseFilter} onChange={(event) => setExpenseFilter(event.target.value)} className="rounded-lg border border-[#dce4df] bg-white px-2 py-1.5 text-xs text-[#35433d]"><option>Todas</option><option>Casa</option><option>Alimentação</option><option>Transporte</option><option>Lazer</option>{customCategories.map((category) => <option key={category}>{category}</option>)}</select></label></div><div className="mt-5 space-y-2">{visibleExpenses.map((expense) => { const Icon = expense.icon; return <div key={expense.title} className="flex items-center justify-between rounded-xl px-2 py-3 hover:bg-[#f8faf7]"><div className="flex items-center gap-3"><div className={`flex h-10 w-10 items-center justify-center rounded-xl ${expense.color}`}><Icon size={18} /></div><div><p className="text-sm font-semibold text-[#35433d]">{expense.title}</p><p className="text-xs text-[#9aa29e]">{expense.category} · {expense.date}</p></div></div><p className="text-sm font-bold text-[#35433d]">{formatCurrency(expense.amount)}</p></div> })}</div></div>
        </section>}
