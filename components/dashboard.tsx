...
export default function Page({ initialProfileName, initialHouseholdName, initialAvatarImage }: DashboardProps) {
  ...
  function exportExpenses() {
    ...
    const rows = expenseItems.map((item) => [item.title, item.category, item.date, item.amount.toFixed(2)].map((value) => `"${value.replaceAll('"', '""')}"`).join(','))
    const blob = new Blob([[header, ...rows].join('\\n')], { type: 'text/csv;charset=utf-8;' })
    const url = URL.createObjectURL(blob)
    const link = document.createElement('a')
    link.href = url
    link.download = 'gastos-junho.csv'
    link.click()
