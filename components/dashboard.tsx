...
export default function Page({ initialProfileName, initialHouseholdName, initialAvatarImage, initialAccentColor, initialTheme, initialAccountCreatedAt, initialExpenses, initialSavings, initialPayableBills }: DashboardProps) {
  ...
  const monthlyTotals = useMemo(() => Array.from({ length: 6 }, (_, index) => { const date = new Date(currentDate.getFullYear(), currentDate.getMonth() - (5 - index), 1); const total = expenseItems.filter((item) => { const spent = new Date(item.date); return spent.getFullYear() === date.getFullYear() && spent.getMonth() === date.getMonth() }).reduce((sum, item) => sum + item.amount, 0); return { label: date.toLocaleDateString('pt-BR', { month: 'short' }).replace('.', ''), total } }), [currentDate, expenseItems])
  ...
  function exportAccountData() {
    const payload = { exportedAt: new Date().toISOString(), profile: { name: profileName, household: groupNames }, expenses: expenseItems.map(({ id, title, category, date, amount, paidBy }) => ({ id, title, category, date, amount, paidBy })), savings, bills }
    const blob = new Blob([JSON.stringify(payload, null, 2)], { type: 'application/json;charset=utf-8' })
    const url = URL.createObjectURL(blob)
    const link = document.createElement('a')
    link.href = url
    link.download = `dados-financeiros-${new Date().toISOString().slice(0, 10)}.json`
    link.click()
    ...
    <main className="min-h-screen bg-[var(--app-bg)] text-[var(--app-text)]">
      ...
      <div className="mx-auto flex max-w-[1320px]">
        ...
        {activeTab === 'overview' && <section className="min-w-0 flex-1 px-4 pb-24 pt-6 sm:px-5 sm:pb-12 sm:pt-8 lg:px-10 lg:pt-12">
          <div className="mb-9 flex flex-col justify-between gap-4 sm:flex-row sm:items-end"><div><p className="mb-2 text-sm font-medium text-[#8a938f]">{currentDate.toLocaleDateString('pt-BR', { weekday: 'long', day: 'numeric', month: 'long', year: 'numeric' })}</p><h1 className="text-3xl font-bold tracking-[-0.04em] text-[#203f36] sm:text-[38px]">{timeGreeting}, {profileName.split(' ')[0]}.</h1><p className="mt-2 flex items-center gap-2 text-sm font-medium text-[#8a938f]"><Clock3 size={15} aria-hidden="true" />Agora são {formattedTime}</p></div><button onClick={() => setShowExpense(true)} className="flex w-fit items-center gap-2 rounded-xl bg-[var(--accent-user)] px-4 py-3 text-sm font-bold text-[#203f36] shadow-[0_5px_15px_rgba(141,183,61,.18)] transition hover:-translate-y-0.5"><Plus size={17} /> Adicionar gasto</button></div>
